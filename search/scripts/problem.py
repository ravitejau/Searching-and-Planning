...
class State:
    ...
    def __str__(self):
        return "Node: (x, y, orientation): " + str(self.x) + ", " + str(self.y) + ", " + str(self.orientation)
...
def get_successor(state, action):
    ...
    try:
        ...
        return State(response.x, response.y, response.direction), response.g_cost, response.valid
